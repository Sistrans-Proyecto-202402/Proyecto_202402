...
public interface ProveedorSucursalRepository extends JpaRepository<ProveedorSucursal, ProveedorSucursalPK>{
    @Query(value = "SELECT * FROM proveedorsucursal", nativeQuery = true)
    ...
    ProveedorSucursal findProveedorSucursalById(@Param("sucursalId") Long sucursalId, @Param("proveedorId") Long proveedorId);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO proveedorsucursal (sucursal_id, proveedor_id) VALUES (:sucursalId, :proveedorId)", nativeQuery = true)
    void insertProveedorSucursal(@Param("sucursalId") Long sucursalId,
